...
async function buildScene() {
    ...
    gltfLoader.load("/models/ship_lod_high.gltf", (gltf) => {
    ...
    gltfLoader.load(
        ...
        (gltf) => {
            ...
            for (let i = 0; i < shipCount; i++) {
                ships.setColorAt(i, defaultColor);
            }
            ...
            const genShips = () => {
                ...
                for (let i = 0; i < waterPoints.length; i++) {
                    ...
                    ships.setColorAt(i, getShipColor(shipsData[i], i));
                }
                ships.instanceColor!.needsUpdate = true;
                planet.add(ships);
            };
            genShips();
        },
    ...
    function render(time: number) {
        ...
        if (selectedShipIndex !== null && scrollPos > 0.5) {
            const si = shipsData[selectedShipIndex];
            let url = si.code_url;
            try {
...
<br />
<span>${si.code_url && si.code_url !== 'null' ? `<a href="${si.code_url}" target="_blank">Repo</a>` : ''}${si.code_url && si.code_url !== 'null' && si.demo_url && si.demo_url !== 'null' ? ' - ' : ''}${si.demo_url && si.demo_url !== 'null' ? `<a href="${si.demo_url}" target="_blank">Demo</a>` : ''}</span>
<br />
<span>${si.country && si.country !== 'null' ? `From ${si.country}.` : ""}${si.hours && si.hours !== 'null' ? ` Took ${si.hours} hours.` : ""}</span>
`;
        } else {
            document.getElementById("details")!.innerHTML = '';
        }
